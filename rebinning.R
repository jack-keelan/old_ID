...
row.names <- c(1:1024)
trans_mat <- matrix(ncol = 3, nrow = 1024, 
                        dimnames=list(row.names,column.names))
...
renorm_Cs <- matrix(ncol = 2, nrow = 1022, 
                        dimnames=list(row.names,column.names))
renorm_Cs[,1] <- 1:1022
renorm_Cs[,2] <- 1e02*AveBenchtest[,3]
...
#    count <- 0
    total <- total + 1
    Cs_temp[total] <- (i-1)+(0.5/renorm_Cs[i,2])
    for(j in 1:renorm_Cs[i,2]){
#      print("what    ")
#      print(i)
      total <- total + 1
      Cs_temp[total] <- ((i-1)+(0.5/renorm_Cs[i,2]) + j/renorm_Cs[i,2])
#      count <- count + 1
...
Cs_res <- (na.omit(Cs_temp_res) )  # WTF is this doing???????

Cs_temp2 <- 0
Cs_temp2 <- rbind(Cs_temp,Cs_temp_res)
dim(Cs_temp2)

Cs_temp3[i,j] <- 0
for(i in 1:total){
#  Cs_temp3[i,1] <- i
  if(!is.na(Cs_temp_res[i])) Cs_temp3[i] <- Cs_temp_res[i]
}

column.names <- c("bin","res_bottom","counts")
row.names <- c(1:256)
new_Cs <- matrix(ncol = 3, nrow = 256, 
                    dimnames=list(row.names,column.names))
new_Cs[,1] <- 1:256
new_Cs[,3] <- 0
new_Cs[1,2] <- max(na.omit(Cs_temp_res) )
for(i in 2:256){
  new_Cs[i,2] <- new_Cs[1,2] - res_width_Cs*(i-1)
}

count <- 0
for(j in 1:256){
  for(i in 1:total){
    if(is.na(Cs_temp_res[i])) count <- count + 1 
    else {
      if(Cs_temp_res[i] >= new_Cs[j,2]){
        next
      }
      new_Cs[j,3] <- new_Cs[j,3] + 1
    }
  }
}
...
#for(j in 1:256){
  for(i in 1:total){
    if(is.na(Cs_temp_res[i])) count <- count + 1 
      ...
      flag <- any(Cs_temp_res[i] <= Cs_temp_res[i]-res_width_Cs){
        next
      }
      new_Cs[j,3] <- new_Cs[j,3] + 1
    }
...
#}
#x=(beta/exp(y_1/y_scale))
column.names <- c("bin","res_bottom","counts","energy")
row.names <- c(1:256)
new_Cs2 <- matrix(ncol = 4, nrow = 256, 
                 dimnames=list(row.names,column.names))

new_Cs2[,1:3] <- new_Cs[,1:3]
for(i in 1:256){
  new_Cs2[i,4] <- beta/exp(new_Cs[i,2]/y_scale)
}
plot(new_Cs2[,4],new_Cs2[,3])
...
hist(rpois(10000000,50), breaks = 20) #,breaks = "Sturges")
## energy resolution
